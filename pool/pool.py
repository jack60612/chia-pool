...
class Pool:
    ...
    async def init_pps_wallet_rpc(self):
        ...
        res = await self.pps_wallet_rpc_client.get_wallet_balance(self.pps_wallet_id)
    ...
    async def stop(self):
        ...
        self.pps_wallet_rpc_client.close()
        await self.pps_wallet_rpc_client.await_closed()
    ...
    async def get_and_validate_singleton_state(
        ...
        if singleton_tip_state.target_puzzle_hash != self.default_target_puzzle_hash:
            ...
            if self.state_keeper.blockchain_state["peak"].height - coin_record.confirmed_block_index > \
