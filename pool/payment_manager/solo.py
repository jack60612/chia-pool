...
class SoloPaymentManager(AbstractPaymentManager):
    """
    Solo payment manager
    Solo Pooling
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Interval for making payout transactions to solo farmers
        ...
        self.solo_fee = self._pool_config["solo_fee"]
    ...
    async def create_payment_loop(self):
        """
        Calculates the points of each solo farmer, and splits the total funds received rewards for each farmer
        ...
        while True:
            try:
                ...
                if len(coin_records) == 0:
                    self._logger.info("No SOLO funds to distribute.")
                    await asyncio.sleep(120)
                    continue

                total_amount_claimed = sum([c.coin.amount for c in coin_records])
                pool_coin_amount = int(total_amount_claimed * self.solo_fee)
