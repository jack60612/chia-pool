...
class DefaultPaymentManager(AbstractPaymentManager):
    ...
    async def pps_submit_payment_loop(self):
        while True:
            try:
                peak_height = self._state_keeper.blockchain_state["peak"].height
                await self._pps_wallet_rpc_client.log_in_and_skip(fingerprint=self.pps_wallet_fingerprint)
                if not self._state_keeper.blockchain_state["sync"]["synced"] or not self._state_keeper.wallet_synced:
                    self._logger.warning("Waiting for wallet sync")
                    await asyncio.sleep(60)
                    continue
                ...
                blockchain_fee: uint64 = uint64(0)
                try:
                    transaction: TransactionRecord = await self._pps_wallet_rpc_client.send_transaction_multi(
                ...
                while (
                    ...
                    transaction = \
                        await self._pps_wallet_rpc_client.get_transaction(self.pps_wallet_id, transaction.name)
                    peak_height = self._state_keeper.blockchain_state["peak"].height
                    self._logger.info(
